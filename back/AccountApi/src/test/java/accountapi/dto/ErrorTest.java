...
class ErrorTest {
    ...
    void testErrorToString() {
        Error error = new Error().code("500").message("Internal").details("Something went wrong");
        String str = error.toString();

        assertNotNull(str);
        assertTrue(str.contains("class Error"));
        assertTrue(str.contains("500"));
        assertTrue(str.contains("Internal"));
    }
    ...
    @Test
    void testErrorEqualsDifferentFields() {
        Error base = new Error().code("400").message("Bad").details("detail");
        Error diffMessage = new Error().code("400").message("Other").details("detail");
        Error diffDetails = new Error().code("400").message("Bad").details("other");

        assertNotEquals(base, diffMessage);
        assertNotEquals(base, diffDetails);
    }
}
